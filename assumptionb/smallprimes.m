...
prlists := [primes(GF(2, d)) : d in [1..8]];
prlists := [&join prlists[1..d] : d in [1..8]];
...
assert prlists[8] eq Set(PrimesUpTo(47)) join {59, 61, 67, 71, 73, 113, 127, 131, 137, 139, 241, 257};

//

// We need to check for p = 43, 67, 73 that the positive-rank simple factors
...
for p in [43, 67, 73]
 do assert #[M : M in NewformDecomposition(CuspidalSubspace(ModularSymbols(Gamma1(p),2,1)))
           | IsZero(WindingElementProjection(M))]
